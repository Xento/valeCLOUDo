...
SmarthomeAPI.prototype.login = function (username, password) {
  ...
  return new Promise(function(resolve, reject) {
    ...
    lock.acquire("login", function(done) {
...
SmarthomeAPI.prototype.updateMap = function(dontRetry) {
  ...
  if(dontRetry == true && servicetoken != undefined) {
    console.error(new Date(), "Can't get update map because not logged in");
    return;
  }
  ...
  self.vacuum.getMapV1(function(error, mapRes){
    ...
    if(error) {
      console.error(new Date(), "Error when getting new map from robot: "+error);
      return;
    }

    if(mapRes != undefined && mapRes[0] != 'retry' && mapRes[0] != 'locating'){
      //console.debug(new Date(), "Xiaomi Cloud: got map name: "+mapRes[0]);
      var unixtime = Math.floor(new Date() / 1000);
      if(mapUrlCache[mapRes[0]] == undefined || mapUrlCache[mapRes[0]].expires < (unixtime - 60)){
        
        //console.debug(new Date(), "Xiaomi Cloud: Updating map url");
        ...
        //console.debug(new Date(), "Xiaomi Cloud: Downloading map from cached url. Expires: "+mapUrlCache[mapRes[0]].expires+" Current: "+unixtime);
        downloadMap.call(this, mapUrlCache[mapRes[0]].url);
      }
    }
    else {
      console.error(new Date(), "Xiaomi Cloud: Failed to get map url name:" + mapRes);
    }
  });
}
...
function getMapURL(mapName) {
  ...
  return new Promise(function(resolve, reject) {
    ...
    if(servicetoken == undefined || servicetoken == null || servicetoken == "null"){
      reject("Xiaomi Cloud: Can't get map, not logged in");
      return;
    }
...
*/
  request(options, function (error, response, body) {
    //console.log(response.headers);
  }).pipe(request.put("http://127.0.0.1/api/miio/map_upload_handler_0"));
}
