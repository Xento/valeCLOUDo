...
MqttClient.prototype.connect = function () {
	if (!this.client || (this.client && this.client.connected === false && this.client.reconnecting === false)) {
		...
		this.topics = {
			...
			homeassistant_autoconf_battery_sensor: this.autoconfPrefix + "/sensor/" + this.topicPrefix + "_" + this.identifier + "_battery/config",
		...
		this.autoconf_payloads = {
			vacuum: {
				...
				supported_features: [
					...
					"stop",
					"return_home",
					// "battery", // entfernt: Battery-Feature ist bei HA Vacuum deprecated
					"status",
					"locate",
					"clean_spot",
					"fan_speed",
			...
			battery_sensor: {
				name: this.identifier + " Battery",
				unique_id: this.identifier + "_battery",
				device: { manufacturer: "Roborock", model: this.vacuum.model, name: this.deviceName, identifiers: [ this.identifier ], sw_version: this.version },
				state_topic: this.topics.state,
				...
				// Optional: availability_topic falls vorhanden, z.B. this.topicPrefix + "/" + this.identifier + "/connected"
			}
		};
		...
		this.connectCallback = () => {
			...
			this.client.subscribe([
				...
				if (!err) {
					...
					// NEU: separater Battery-Sensor nach HA Discovery
					this.client.publish(this.topics.homeassistant_autoconf_battery_sensor, JSON.stringify(this.autoconf_payloads.battery_sensor), {
						retain: true, qos:this.qos
					});
					this.updateAttributesTopic();
				} else {
