        ...
        if(decodedResponse.msg) {
            if(self.pendingRequests[decodedResponse.msg.id]) {
                ...
                if (decodedResponse.msg.error) {
                    self.pendingRequests[decodedResponse.msg.id].callback(decodedResponse.msg.error.message || "unspecified error");
                } else {
                    self.pendingRequests[decodedResponse.msg.id].callback(null, decodedResponse.msg.result || "unspecified result");
                }
...
Vacuum.prototype.setToken = function(token) {
    if (!(this.token.equals(token))) {
        console.info(new Date(), "Changing to new token.");
        this.token = token;
...
Vacuum.prototype.setIP = function(ip){
    if (this.ip != ip) {
        console.info(new Date(), "Changing to new ip: "+ip);
        this.ip = ip;
    }
    // also reset a stamp just in case
    this.stamp = new Stamp({});
            ...
            response.forEach(function(elem){
                if(!Array.isArray(elem) || (Array.isArray(elem) && elem.length < 3)){
                    ...
                    timers.push({
                        ...
                        fanpower: elem[2][1][1] && elem[2][1][1]["fan_power"],
                        segments: elem[2][1][1] && elem[2][1][1]["segments"],
                        iterations: elem[2][1][1] && elem[2][1][1]["repeat"]
                    })
                }
            });
            if(err) {
...
Vacuum.prototype.addTimer = function(settings, callback) {
    this.getCurrentStatus((err,res) => {
        if (!err && res.msg_ver === 3) {
            this.sendMessage("set_timer", [[settings.id, [settings.cron, ["start_clean", {"fan_power": settings.fanpower, "segments": settings.segments, "repeat": settings.iterations, "clean_order_mode": 1}]]]], {}, callback);
        } else {
            this.sendMessage("set_timer", [[settings.id, [settings.cron, ["",""]]]], {}, callback);
        }
    });
};
...
Vacuum.prototype.getMapV1 = function(callback) {
    this.sendMessage("get_map_v1", [], {}, callback);
};
...
Vacuum.prototype.nameSegment = function(names, callback) {
    if(Array.isArray(names)) {
        ...
        this.sendMessage("name_segment", names, {timeout: 2000}, callback);
    } else {
...
Vacuum.prototype.mergeSegment = function(segments, callback) {
    if(Array.isArray(segments) && segments.length === 2) {
        this.sendMessage("merge_segment", segments, {timeout: 3500}, callback);
    } else {
...
Vacuum.prototype.splitSegment = function(splitBy, callback) {
    if(Array.isArray(splitBy) && splitBy.length === 5) {
        ...
        this.sendMessage("split_segment", flippedSplitBy, {timeout: 3500}, callback);
    } else {
