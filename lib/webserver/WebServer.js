...
const WebServer = function (options) {
    ...
    this.smarthomeAPI = options.smarthomeAPI;
    ...
    this.app.put("/api/set_interface_config", function (req, res) {
        ...
        ['zonedImmediate','gotoImmediate','showMultimap','hideMapStatus','hideMapReload','staticMapButtons'].forEach(option => {
            if (req.body[option]) {
                config[option] = true;
            }
        });
    ...
    this.app.post("/api/get_smarthomeapi_robots", function (req, res) {
        self.smarthomeAPI.getDeviceStatus(res.req.body.username, res.req.body.password, '{"getVirtualModel":false,"getHuamiDevices":0}').then(
            function(result){
                res.json(result);
    ...
    this.app.get("/api/get_smarthomeapi_config", function (req, res) {
        res.json(self.configuration.get("smarthomeAPI"));
    });

    this.app.put("/api/set_smarthomeapi_config", function (req, res) {
        self.configuration.set("smarthomeAPI",res.req.body);
        self.vacuum.setToken(Buffer.from(res.req.body.token, "hex"));
        self.vacuum.setIP(res.req.body.deviceip);
        res.json({res:"success"});
    });
        ...
        if (req.body && req.body.cron && req.body.id) {
            new Promise((resolve,reject) => {
                if (req.body.edit && req.body.edit.length) {
                    self.vacuum.deleteTimer(req.body.edit, function (err, data) {
                        if (err) {
                            reject(err);
                        } else {
                            resolve(data);
            ...
            .then(() => {
                return new Promise((resolve,reject) => {
                    self.vacuum.addTimer({id: req.body.id, cron: req.body.cron, fanpower: +req.body.fanpower || 102, segments: req.body.segments || "", iterations: +req.body.iterations || 1}, function (err, data) {
                        if (err) {
                            reject(err);
                        } else {
                            resolve(data);
    ...
    var refreshMapIntervalId = undefined;

    setInterval(function() {
        wss.clients.forEach(function each(ws) {
            //terminate inactive ws
            if (ws.isAlive === false) {
                console.info(new Date(),"Webclient disconnected");
                return ws.terminate();
            }
            //mark ws as inactive
            ws.isAlive = false;
        ...
        if (!wss.clients.size) {
            if(refreshMapIntervalId != undefined) {
                console.info(new Date(),"No webclient connected, stopping map-polling.");
                clearInterval(refreshMapIntervalId);
            }
            
            refreshMapIntervalId = undefined;
        }
    }, 30000);
    ...
    wss.on("connection", (ws) => {
        ...
        console.log(new Date(),"Webclient connected");

        if(refreshMapIntervalId == undefined){
            refreshMapIntervalId = 0;

            // update Map on connect
            self.vacuum.getCurrentStatus((err,res) => {
                self.smarthomeAPI.updateMap();
            });

            self.smarthomeAPI.login().then(function(result){
                refreshMapIntervalId = setInterval(function(){
                    self.vacuum.getCurrentStatus((err,res) => {
                        if (!err) {

                            if(self.map.bin){
                                // don't update map when standing still and an old map exists
                                switch(res.state){
                                    case 2:
                                    case 3:
                                    case 8:
                                    ...
                                    case 13:
                                    case 14:
                                        console.debug(new Date(), "Robot not moving: "+res.state);
                                        return;
                                }
                            }
                            ...
                            console.error(new Date(), "Error getting robot status");
                    });
